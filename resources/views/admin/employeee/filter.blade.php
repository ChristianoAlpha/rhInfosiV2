      ...
      <div>
        ...
        @if(isset($filtered) && $filtered->count() > 0)
          <form action="{{ route('admin.employeee.filter.pdf')}}" target="_blank" method="post">
            ...
            <input type="hidden" name="employeeCategoryId"
              value="{{ request('employeeCategoryId', $selectedCategory ?? '') }}">
            <input type="hidden" name="courseId" value="{{ request('courseId', $selectedCourse ?? '') }}">
            <button class="btn btn-outline-light btn-sm me-2" title="Baixar PDF" target="_blank" rel="noopener noreferrer">
            ...
            <div class="form-floating">
              <select name="departmentId" class="form-select">
                ...
                @foreach($departments as $type)
                  <option value="{{ $type->id }}" {{ (request("departmentId", $selectedDepartment ?? "") == $type->id) ? "selected" : "" }}>
                    {{ $type->title }}
                  </option>
                @endforeach
              </select>
            ...
            <div class="form-floating">
              <select name="positionId" class="form-select">
                ...
                @foreach($position as $type)
                  <option value="{{ $type->id }}" {{ (request("positionId", $selectedPosition ?? "") == $type->id) ? "selected" : "" }}>
                    {{ $type->name }}
                  </option>
                @endforeach
              </select>
            ...
            <div class="form-floating">
              <select name="specialityId" class="form-select">
                ...
                @foreach($speciality as $type)
                  <option value="{{ $type->id }}" {{ (request("specialityId", $selectedSpeciality ?? "") == $type->id) ? "selected" : "" }}>
                    {{ $type->name }}
                  </option>
                @endforeach
              </select>
            ...
            <div class="form-floating">
              <select name="employeeTypeId" class="form-select">
                ...
                @foreach($employeeTypes as $type)
                  <option value="{{ $type->id }}" {{ (request("employeeTypeId", $selectedType ?? "") == $type->id) ? "selected" : "" }}>
                    {{ $type->name }}
                  </option>
                @endforeach
              </select>
            ...
            <div class="form-floating">
              <select name="employeeCategoryId" class="form-select">
                ...
                @foreach($employeeCategories as $type)
                  <option value="{{ $type->id }}" {{ (request("employeeCategoryId", $selectedType ?? "") == $type->id) ? "selected" : "" }}>
                    {{ $type->name }}
                  </option>
                @endforeach
              </select>
            ...
            <div class="form-floating">
              <select name="courseId" class="form-select">
                ...
                @foreach($courses as $type)
                  <option value="{{ $type->id }}" {{ (request("courseId", $selectedType ?? null) == $type->id) ? "selected" : "" }}>
                    {{ $type->name }}
                  </option>
                @endforeach
              </select>
