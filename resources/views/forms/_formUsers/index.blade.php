     ...
     <div class="col-md-6">
         <div class="form-floating">
             <select name="role" id="role" class="form-select" required>
                 ...
                 <option value="director" {{ $user->role == 'director' ? 'selected' : '' }}>Diretor</option>
                 <option value="department_head" {{ $user->role == 'department_head' ? 'selected' : '' }}>Chefe de
                     Departamento</option>
                 <option value="hr" {{ $user->role == 'hr' ? 'selected' : '' }}>Área Administrativa (RH)</option>
 ...
 <div id="department_head_fields" style="display: {{ $user->role === 'department_head' ? 'block' : 'none' }};"
     class="mb-4">
                 ...
                 <select name="department_id" id="department_id" class="form-select">
                     ...
                     @foreach ($departments as $dept)
                         <option value="{{ $dept->id }}"
                             {{ $user->department_id == $dept->id ? 'selected' : '' }}>{{ $dept->title }}</option>
                     @endforeach
                 </select>
 ...
 <script>
     function updateEmployeeFields() {
         ...
         if (select.value) {
             ...
             emailField.value = '{{ old('email', $user->email) }}';
             nameField.value = '';
     ...
     function toggleConditionalFields() {
         ...
         document.getElementById('department_head_fields').style.display = (role === 'department_head') ? 'block' :
             'none';
